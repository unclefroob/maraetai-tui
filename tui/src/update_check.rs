...
/// Where this repo lives — hardcoded, since this is a genuinely separate
/// GitHub repository from its macOS counterpart, not a runtime choice.
const REPO_OWNER: &str = "unclefroob";
const REPO_NAME: &str = "maraetai-tui";
...
async fn fetch_latest_commit() -> Result<RemoteCommit> {
    ...
    let resp = client
        .get(&url)
        .header("User-Agent", "maraetai-tui")
...
/// lookup covers a TUI launched from a shell, desktop launcher, or another
/// process with a restricted environment. `CARGO` remains an explicit
/// override.
